    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white">
      <div className="mx-auto max-w-5xl space-y-12 px-6 py-12 md:py-16">
        <header className="grid items-start gap-8 lg:grid-cols-[1.6fr_1fr]">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 text-sm text-indigo-50/90">
            <InfoCard
              eyebrow="Disponibilité"
              title="Stage avril 2026"
              subtitle="Durée : 4-5 mois"
            />
          </div>
        </header>
        ...
        <Section title="Formations">
          <div className="space-y-4">
            {data.Formations.map((formation, index) => (
              <FormationCard key={formation.Nom + index} formation={formation} />
            ))}
          </div>
        </Section>
